...
namespace Xaver.Helper
{
    public static class MessageHandler
    {
        ...
        public static string GetLogMessage(string result, string applicationEntity, string methodName, string serviceEndpoint, string requestMessage, string responseMessage, List<Xaver.Helper.Logger.NextEndpoint> nextEndpoints, string Message)
        {
            ...
            foreach (Xaver.Helper.Logger.NextEndpoint nextEndpoint in nextEndpoints)
            {
        ...
        public static string GetErrorMessage(Exception e, string result, string applicationEntity, string methodName, string serviceEndpoint, string requestMessage, string responseMessage, List<Xaver.Helper.Logger.NextEndpoint> nextEndpoints, string Message)
        {
            string row0 = string.Format("[TransactionResult]  {0}  {1}  {2}  {3}", result, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff"), applicationEntity, methodName);
            string row1 = string.Format("[ServiceEndpoint]\r\n{0}", serviceEndpoint);
            string row2 = string.Format("[RequestMessage]\r\n{0}", requestMessage);
