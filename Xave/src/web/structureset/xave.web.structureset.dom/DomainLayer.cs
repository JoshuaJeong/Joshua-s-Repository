...
namespace xave.web.structureset.dom
{
        ...
        private static ISession Session
        {
            get
            {
                if (session == null || !session.IsOpen)
                {
                    lock (syncRoot)
                    {
                        session = NHibernateHelper.GetSession("StructureSet", new List<Type>() { typeof(BodyStructureMap), typeof(Section_Map), typeof(SectionMapMap), typeof(SectionPartMap), typeof(HeaderMapMap), typeof(HeaderPartMap), typeof(HeaderStructureMap), typeof(DocumentMap) });
                    }
                }
                return session;
            }
